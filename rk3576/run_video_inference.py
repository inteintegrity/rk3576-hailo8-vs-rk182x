"""RK3576 built-in NPU YOLO11n video inference.

Mirrors the Hailo-8 and RK1820 video runners frame for frame: same letterbox, same
shared decode, same annotation and the same reported timing layers, so the three
clips differ only by the accelerator. Driven through rknn-toolkit-lite2 on the
SoC's own NPU (no PCIe hop).

Usage:
    python run_video_inference.py --model yolo11n_rk3576_int8.rknn --video clip.mp4         --out-dir out --core-mask 7
"""
...
from rknn_runner import (  # noqa: E402
    collect_heads, collect_heads_yolo26, dequantize, run_inference, sha256,
)
...
def main() -> None:
    ...
    for index in range(limit):
        ...
        if writer is None:
            writer = cv2.VideoWriter(
                str(args.out_dir / f"rk3576_npu_{tag}_result.mp4"),
    ...
    report = {
        ...
        "timing": {
            ...
            "note": (
                ...
                "same client. The Python figures add letterbox, the rknnlite call, decode, NMS "
    ...
    report_path = args.out_dir / f"rk3576_npu_{tag}_video_result.json"
    ...
    log(f"annotated video: {args.out_dir / f'rk3576_npu_{tag}_result.mp4'}")
    log(f"json report: {report_path}")
    (args.out_dir / "rk3576_npu_video_inference.log").write_text("\n".join(log_lines) + "\n", encoding="utf-8")
